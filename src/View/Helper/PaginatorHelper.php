...
class PaginatorHelper extends Helper
{
     ...
     * - `escape` Whether you want the content HTML entity encoded, defaults to true
     ...
     *
     * @param string $content Content for the link. Defaults to '<< Previous'.
     ...
     */
    public function prev(string $content = '<< Previous', array $options = []): string
    {
        $defaults = [
            'url' => [],
            'disabledTitle' => $content,
            'escape' => true,
        ];
        $options += $defaults;
        $options['step'] = -1;
     ...
     *
     * @param string $content Content for the link. Defaults to 'Next >>'.
     * @param array<string, mixed> $options Options for pagination link. See above for list of keys.
     * @return string A "next" link or $disabledTitle text if the link is disabled.
     * @link https://book.cakephp.org/5/en/views/helpers/paginator.html#creating-jump-links
     */
    public function next(string $content = 'Next >>', array $options = []): string
    {
        $defaults = [
            'url' => [],
            'disabledTitle' => $content,
     ...
     * @param array<string, mixed>|string|null $content Content for the link. If $content is null $key will be used
     ...
     */
    public function sort(string $key, array|string|null $content = null, array $options = []): string
    {
        ...
        if (is_array($content) && array_key_exists($dir, $content)) {
            $content = $content[$dir];
        }
        ...
        $vars = [
            'text' => $options['escape'] ? h($content) : $content,
